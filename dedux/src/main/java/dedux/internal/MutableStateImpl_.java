...
//public class MutableStateImpl_ implements MutableState {
//
...
//
//
//    // we really need to have a way to indicate what full amount of state we have
//    // if we will obtain all the saved values lazily (when asked for) then
//    // if called `state` we might (and most likely will) receive not the full _map_ of
//    // our state, but only small part that had been evaluated (asked before)
//    // so, we need, at least, to add to the storage another method `getAllStates` that we are there
//    // if it's not there then it's the default one, should not be ok
//    // actually, we do not need even `getAll` names, we can just call `getAll`
//    public MutableStateImpl_(@Nullable PreloadedState preloadedState) {
